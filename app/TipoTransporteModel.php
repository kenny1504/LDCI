...
class TipoTransporteModel extends Model
{
    ...
    public function eliminar($id_TipoTransporte, $id_session)
    {
        ...
        $query = DB::UPDATE('UPDATE ldci.tb_tipo_transporte
                                ...
                                WHERE id_tipo_transporte=? AND
                                NOT EXISTS (SELECT c.id_tipo_transporte FROM ldci.tb_cotizacion AS c
                                WHERE c.id_tipo_transporte=?)', [$id_session, $id_TipoTransporte, $id_TipoTransporte]);
