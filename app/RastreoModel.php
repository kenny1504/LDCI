...
class RastreoModel extends Model
{
    ...
    {
        if ($tipoUsuario == 1) {
            $table = "(SELECT f.id_cotizacion,case cl.tipo when 1
                ...
                JOIN ldci.tb_persona AS p ON cl.id_persona=p.id_persona WHERE c.estado=4
                ORDER BY id_cotizacion DESC) as tb";
        } else {
            $table = "(SELECT DISTINCT f.id_cotizacion,UPPER(f.cliente) AS cliente,f.transporte,f.destino, f.origen, f.fecha_llegada FROM(
                        SELECT f.id_cotizacion,case cl.tipo when 1
                            then p.nombre ||' '|| p.apellido1 ||' '|| COALESCE (p.apellido2,'')
                            else cl.nombre_empresa end as cliente,
                            tt.nombre as transporte, cd.ciudad||'/'||cd.pais as destino,
                            co.ciudad||'/'||co.pais as origen, TO_CHAR (f.fecha_entrega,'DD-MM-YYYY') AS fecha_llegada
                            ...
                            JOIN ldci.tb_cliente AS cl ON cl.id_cliente=f.id_cliente
                            JOIN ldci.tb_persona AS p ON cl.id_persona=p.id_persona
                            WHERE c.estado=4 AND c.usuario_grabacion=$id_session) AS f
                            ORDER BY id_cotizacion DESC) as tb";
        }
    ...
    public function getDetalleSeguimiento($id_cotizacion)
    {
        $query = new static;
        $query = DB::select("SELECT ds.id_detalle_seguimiento, ds.fecha :: Date AS fecha_evento, ds.evento, ds.detalle as descripcion
                        ...
                        WHERE f.id_cotizacion= $id_cotizacion AND ds.estado=1");
        return $query;
    }
    ...
    public function getImagen($id_cotizacion)
    {
        $id_flete = new static;
        $id_flete = DB::select('SELECT f.id_flete FROM ldci.tb_flete AS f WHERE f.id_cotizacion = ?', [$id_cotizacion]);
        $query = new static;
        $query = DB::select('select i.nombre,i.url from ldci.tb_detalle_imagen det
                            join ldci.tb_imagen i on det.id_imagen=i.id_imagen
                            where id_tabla=? and id_tipo=2', [$id_flete[0]->id_flete]);
    ...
    /** */
    public function infoRastreo($id_cotizacion)
    {
        $query = new static;
        $query = DB::select("SELECT f.id_flete, f.fecha_entrega as fecha FROM ldci.tb_flete AS f where f.id_cotizacion = $id_cotizacion");
        return $query;
    }
}
